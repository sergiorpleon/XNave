 package cu.juego.implementacion.nave;

import cu.juego.implementacion.Pixmap;
import cu.juego.implementacion.Sonido;

public class Assets {
public static Pixmap fondo;
public static Pixmap fondo1;
public static Pixmap menuprincipal;
public static Pixmap botones;
public static Pixmap botones1;
public static Pixmap ayuda1;
public static Pixmap ayuda2;
public static Pixmap ayuda3;
public static Pixmap numeros;
public static Pixmap preparado;
public static Pixmap menupausa;
public static Pixmap finjuego;

public static Pixmap logotren;
public static Pixmap autor;
public static Pixmap lineas;
public static Pixmap lineas2;
public static Pixmap nave;
public static Pixmap meteorito;
public static Pixmap bola;
public static Pixmap diamante;
public static Pixmap metNEGRO;
public static Pixmap metNARANJA;
public static Pixmap metROJO;
public static Pixmap enemigo;
public static Pixmap enemigo1;
public static Pixmap diamantegrande;
public static Pixmap meteoritogrande;
public static Pixmap navegrande;
public static Pixmap enemigogrande;
public static Pixmap blanco;
public static Pixmap blanco1;
public static Pixmap niebla;

public static Pixmap award;

public static Sonido na;
public static Sonido nav;
public static Sonido ataque;
public static Sonido diaman;
public static Sonido pulsar;
public static Sonido derrota;

}
